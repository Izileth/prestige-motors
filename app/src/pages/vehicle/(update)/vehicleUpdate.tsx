...
import type { VehicleFormValues } from "../(create)/vehicleCreate";
import { vehicleFormSchema } from "../(create)/vehicleCreate";
import type { UseFormReturn } from "react-hook-form";
import type { Resolver } from "react-hook-form";

import type { VehicleImage } from "~/src/store/slices/vehicle";

import { FuelType, TransmissionType, BodyType,VehicleCategory, VehicleClass } from "../(create)/vehicleCreate";
...
export function EditVehiclePage() {
    ...
    useEffect(() => {
        if (currentVehicle) {
            const formData: VehicleFormValues = {
            ...
            imagens: currentVehicle.imagens || [], // Mantém a estrutura completa
            };
    ...
    const onSubmit = async (data: VehicleFormValues) => {
            if (!id) return;
            console.log('Form data:', data); // ← Adicione isto
        
            try {
            setIsSubmitting(true);
        
            // 1. Filtrar imagens - separar strings (URLs) de objetos File
            const existingImages = data.imagens?.filter((img): img is VehicleImage => 
                typeof img === 'object' && 'url' in img
            ) || [];
        
            const newImages = data.imagens?.filter((img): img is File => 
                img instanceof File
            ) || [];
        
            // 2. Atualizar dados básicos do veículo
            ...
            // 3. Upload de novas imagens (arquivos)
            if (newImages.length > 0) {
                await uploadVehicleImages(id, newImages);  // Certifique-se que esta função aceita File[]
            }
        
            // 4. Lógica para imagens removidas (opcional)
            const currentImageUrls = existingImages.map(img => img.url);
            const imagesToRemove = currentVehicle?.imagens
                ?.filter(img => !currentImageUrls.includes(img.url))
                || [];
        
            await Promise.all(
                imagesToRemove.map(img => 
                deleteVehicleImage(id, img.id)  // Usar img.id em vez de img.url
                )
            );
